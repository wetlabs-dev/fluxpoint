...
async function main() {
  ...
  await prisma.waterParameterReading.deleteMany();
  await prisma.aquariumEvent.deleteMany();
  await prisma.equipmentProfile.deleteMany();
  await prisma.itemTransfer.deleteMany();
  await prisma.aquariumItem.deleteMany();
  await prisma.speciesDefinition.deleteMany();
  await prisma.aquariumProfile.deleteMany();
  ...
  const species = await Promise.all([
    prisma.speciesDefinition.create({
      data: {
        category: "FISH",
    ...
    tankNames.map((generatedName, index) =>
      prisma.aquarium.create({
        data: {
          ...
          heightInches: [18, 12, 16, 10, 12, 18][index],
          ...
          notes: "Seeded for first-version UX and data modeling.",
      ...
      data: {
        ...
        steps: {
          create: steps.map((title, index) => ({
            ...
            config: JSON.stringify({ required: index < 2 })
          }))
        }
      }
    });
  ...
  await prisma.auditLog.create({
    data: {
      ...
      action: "SEED",
      createdById: user.id,
      after: JSON.stringify({ aquariums: aquariums.length, species: species.length })
    }
  });
}
