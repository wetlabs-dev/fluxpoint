...
async function sendDueAlerts() {
  ...
  return { summary: `Scanned ${result.care.scanned} due care tasks; ${result.workflows.scanned} workflow alert(s); ${result.metrics.abnormal} abnormal metrics; ${result.server.incidents} server incidents.`, metadata: result };
}
