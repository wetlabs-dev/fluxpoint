...
Eddy is Fluxpoint's built-in aquarium assistant. Earlier scaffolding referred to this feature as Current Keeper; Eddy is the canonical name going forward.
...
The mock provider is the development-safe default. `AI_ENABLED`, `AI_PROVIDER`, `AI_IMAGE_ENABLED`, and `AI_MODERATION_ENABLED` control the provider status shown in Server Maintenance. OpenAI requests use `OPENAI_API_KEY` and the configured response, image, and moderation models.
