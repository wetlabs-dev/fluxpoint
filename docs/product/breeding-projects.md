...
- **Traits** are observed characteristics only. New trait libraries belong to Species Variants so line-specific traits stay with the morph, locality, strain, or cultivar being bred. Legacy species-level trait records remain readable, and projects record trait expressions with confidence and notes.
...
Breeding projects link to aquariums, species definitions, optional species variants, inventory parents, conditions, timeline events, media assets, care tasks, workflow runs, labels through inventory graduation, and audit logs.
...
When a cohort is established, the keeper can graduate it into Inventory. Graduation creates an `AquariumItem` with `originBreedingProjectId`, `originBreedingCohortId`, the parent species definition, and the project variant when one is set. It updates the cohort stage to `GRADUATED`, writes an aquarium timeline event when a tank is known, and records an audit event. Inventory detail pages show the originating project and cohort.
