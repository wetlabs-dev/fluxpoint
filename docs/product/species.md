...
Optional reference links are available for Wikipedia, iNaturalist, and GBIF on every species category. Plants also show Plants of the World Online (POWO). Blank links are valid. Entered links must be complete HTTP or HTTPS URLs; HTTPS is preferred. Reference links are shown on species cards and the species husbandry workspace when present. Eddy Magic Fill only applies URLs that the server can verify against the accepted taxon and plausible organism group; otherwise the field remains blank. If an older non-plant record has a POWO value, Fluxpoint preserves it on edit but hides it from normal non-plant forms and cards.

Species environmental ranges include temperature, pH, GH, KH, TDS, and salinity. TDS is stored as structured species-level `tdsMin`/`tdsMax` values in ppm because it is part of the organism's environmental profile, not a husbandry note. Eddy Parameter Advisor and other compatibility contexts can use these ranges when comparing tank targets.
