...
Aquarium Intelligence adds persisted parameter analysis for supported water metrics. The engine uses saved readings, aquarium targets, source type, minimum observation counts, minimum time spans, and water-change markers to classify drift, instability, and concern state. Sparse data returns insufficient instead of implying stability, and trend language stays approximate.
