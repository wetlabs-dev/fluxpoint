...
The browser enqueues this work and polls its authenticated job status. The AI worker performs generation, persistence, moderation, and optimistic assignment. If the cover changes after enqueue, the new image remains available but is not auto-applied.
