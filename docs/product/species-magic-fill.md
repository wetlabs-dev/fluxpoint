...
The strict draft contract includes every field group the form can save. Identity covers category, common and scientific components, normalized display name, and author citation. If Eddy can resolve only a genus, the species epithet is explicitly set to `sp.` and a warning explains that the exact species was not resolved. References cover Wikipedia, iNaturalist, GBIF, and plant-only POWO. Non-plant drafts return no POWO URL. The profile covers category-relevant dimensions, fish maximum size, plant CO₂ requirement, life history, aquarium care ranges, hardness, species-level TDS min/max, flow, breeding, salinity, and notes. Eddy may suggest a reference only when it is confident the URL identifies the intended taxon; uncertain URLs remain null. Applying a draft updates reference inputs, TDS inputs, salinity controls, plant CO₂ controls, and derived freshwater/brackish/marine badges immediately.

If the supplied name appears to be a color morph, strain, locality line, cultivar, domestic form, or trade name, Eddy returns the accepted parent species in the canonical fields and adds a review-only `variantSuggestion`. The review panel tells the keeper to save or use the parent species first, then create the suggested variant under that species. Applying the draft never creates a Species Variant automatically.

After the provider returns a draft, Fluxpoint runs a server-side reference verifier and resolver for medium- and high-confidence species-level identities. Model-proposed URLs are first checked for successful direct source pages, accepted taxon/name agreement, and plausible organism group; search pages, 404s, wrong groups, and unverified URLs are removed. The deterministic resolver then treats Wikipedia as a hub, using the accepted article and Wikidata taxon identifiers when available, and asks GBIF and iNaturalist directly; plant drafts can also accept POWO identifiers from Wikidata. If a source cannot be verified, the URL stays blank rather than risking a wrong link.
...
To extend Magic Fill later, add a durable field to the Prisma species model and migration, expose it on the species form and action, then add the matching nullable property to the input schema, draft schema, strict provider JSON schema, review mapping, mock fixture, and integrity check. If the field is instead guide-specific, add it to the husbandry registry so Husbandry Magic Fill can request and apply it visibly. Regional fields belong in the collection-scoped status model rather than on the global species definition. Keeping those layers together prevents the AI contract from drifting away from what Fluxpoint can actually save.
