    ...
    <section className="min-w-0 overflow-hidden rounded-lg border border-primary/35 bg-primary/5 p-4 md:col-span-2" aria-label="Eddy Species Magic Fill">
      ...
      {result ? (
        <div className="mt-4 min-w-0 space-y-4 overflow-hidden rounded-md border border-border bg-background/80 p-4">
          ...
          {result.draft.variantSuggestion ? <div className="rounded-md border border-primary/25 bg-primary/10 p-3 text-xs"><p className="font-semibold">Suggested variant · {result.draft.variantSuggestion.name ?? "Unnamed variant"}</p><p className="mt-1 text-muted-foreground">Parent: {result.draft.variantSuggestion.parentCommonName ?? result.draft.canonical.commonName ?? "parent species"}{result.draft.variantSuggestion.parentScientificName ? ` · ${result.draft.variantSuggestion.parentScientificName}` : ""}</p><p className="mt-2">{result.draft.variantSuggestion.reason ?? "This looks like a named variant under the parent species, not a separate species definition."}</p><p className="mt-2 text-muted-foreground">After saving the parent species, add this under the Variants section.</p></div> : null}
