...
export function EquipmentForm({ aquariums, sources, lightCapabilities, item }: any) {
  ...
  return <form action={item ? updateEquipment : createEquipment} className="mt-4 grid gap-6">
    ...
    {type === "LIGHT" ? <Section title="Light capability" description="Match this fixture to the channels its controller exposes."><Field label="Capability profile" wide><Select name="lightCapabilityProfileId" defaultValue={profile?.lightCapabilityProfileId ?? ""}><option value="">Not a controllable light</option>{lightCapabilities.map((c: any) => <option key={c.id} value={c.id}>{c.name}</option>)}</Select></Field></Section> : <input type="hidden" name="lightCapabilityProfileId" value="" />}
