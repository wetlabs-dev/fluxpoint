...
export async function InventoryDetailWorkspace({ id, view = "overview", equipmentOnly = false }: { id: string; view?: string; equipmentOnly?: boolean }) {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Fact label="Placement" value={placement} /><Fact label="Species" value={item.speciesDefinition?.commonName ?? "Not linked"} /><Fact label="Acquired" value={item.acquiredAt ? format(item.acquiredAt, "MMM d, yyyy") : "Not recorded"} /><Fact label="QR code" value={qr?.publicCode ?? "Not generated"} mono /></div>
...
function LightLoad({ item }: { item: any }) { const rows = item.lightingAssignments.map((assignment: any) => { if (!assignment.enabled) return `${assignment.aquarium?.generatedName ?? assignment.aquarium?.name ?? "Aquarium"}: disabled`; if (!assignment.schedule) return `${assignment.aquarium?.generatedName ?? assignment.aquarium?.name ?? "Aquarium"}: no schedule`; const result = calculateScheduleLightLoad(assignment.schedule.points, assignment.schedule.capabilityProfile, item.equipmentProfile); return `${assignment.aquarium?.generatedName ?? assignment.aquarium?.name ?? "Aquarium"}: ${assignment.schedule.name} · ${result.displayValue}${result.outputMethod === "WATTAGE_ESTIMATED" ? ` (${result.confidence.toLowerCase()} confidence, estimated from wattage)` : ""}`; }); return <Info label="Estimated daily light load" value={rows.length ? rows.join("\n") : "Assign this light to an aquarium and schedule"} />; }
