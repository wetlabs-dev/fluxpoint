...
import { createMaintenanceEvent, duplicateEquipment, logInhabitantLoss, markEquipmentMaintained, transferItem } from "@/domains/management/actions";
...
export async function InventoryDetailWorkspace({ id, view = "overview", equipmentOnly = false }: { id: string; view?: string; equipmentOnly?: boolean }) {
  ...
  const item = await prisma.aquariumItem.findFirst({
    ...
    include: {
      ...
      originBreedingProject: true,
      originBreedingCohort: true,
  ...
  const attachedAquariums = item.aquariumAttachments.map((entry) => entry.aquarium.name).join(" · ");
  const vesselAquariums = item.aquariumAttachments.filter((entry) => entry.role === "AQUARIUM_VESSEL").map((entry) => entry.aquarium.name).join(" · ");
  const sharedWarning = item.itemType === "EQUIPMENT" && item.aquariumAttachments.length > 1 && !item.equipmentProfile?.multiAquariumCapable;
  const placement = (item.aquarium?.name ?? attachedAquariums) || item.storageLocation?.name || item.quarantineProject?.name || item.status.toLowerCase().replaceAll("_", " ");
  const sexBreakdown = formatFishSexBreakdown(item);
  return <div className="space-y-6">
    <PageHeader title={item.name} eyebrow="Inventory detail"><div className="flex flex-wrap gap-2"><Badge>{item.itemType.toLowerCase()}</Badge><Badge>{item.status.toLowerCase().replaceAll("_", " ")}</Badge>{item.itemType === "EQUIPMENT" && item.equipmentProfile?.multiAquariumCapable ? <Badge>Shared equipment</Badge> : null}{sharedWarning ? <Badge className="bg-amber-100 text-amber-950 dark:bg-amber-900/35 dark:text-amber-100">Multi-tank warning</Badge> : null}<Badge>{item.quantity} {item.unit ?? "units"}</Badge></div></PageHeader>
    {sharedWarning ? <div className="rounded-md border border-amber-300 bg-amber-50 p-4 text-sm font-semibold text-amber-950 dark:border-amber-700 dark:bg-amber-950/30 dark:text-amber-100">Assigned to multiple aquariums without the shared-equipment flag.</div> : null}
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Fact label="Placement" value={placement} /><Fact label="Species" value={item.speciesVariant ? `${item.speciesVariant.displayName ?? item.speciesVariant.name} · ${item.speciesDefinition?.commonName ?? "parent"}` : item.speciesDefinition?.commonName ?? "Not linked"} /><Fact label="Acquired" value={item.acquiredAt ? format(item.acquiredAt, "MMM d, yyyy") : "Not recorded"} /><Fact label={sexBreakdown ? "Sex breakdown" : "QR code"} value={sexBreakdown ?? qr?.publicCode ?? "Not generated"} mono={!sexBreakdown} /></div>
    ...
    {currentView === "overview" ? <section className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_420px]"><Card><CardHeader><CardTitle><Package className="mr-2 inline h-5 w-5" />Overview</CardTitle></CardHeader><CardContent className="grid gap-4 sm:grid-cols-2"><Info label="Name" value={item.name} /><Info label="Type" value={item.itemType} /><Info label="Status" value={item.status} /><Info label="Quantity" value={`${item.quantity} ${item.unit ?? "units"}`} />{sexBreakdown ? <Info label="Sex breakdown" value={sexBreakdown} /> : null}<Info label="Source / vendor" value={item.source?.name ?? item.acquiredFrom} /><Info label="Unit price" value={item.purchasePrice ? `$${item.purchasePrice}` : null} /><Info label="Variant / line" value={item.speciesVariant?.displayName ?? item.speciesVariant?.name} /><Info label="Scientific name" value={item.speciesDefinition?.scientificName} /><Info label="Aliases" value={item.speciesDefinition?.aliases.map((entry) => entry.alias).join(" · ")} />{item.originBreedingProject ? <div className="rounded-md bg-muted/45 p-3"><div className="text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">Breeding origin</div><Link href={`/breeding/${item.originBreedingProject.id}`} className="mt-1 block text-sm font-semibold text-primary underline">{item.originBreedingProject.title}</Link>{item.originBreedingCohort ? <div className="text-xs text-muted-foreground">{item.originBreedingCohort.name}</div> : null}</div> : null}<Info label="Description" value={item.description} /><Info label="Notes" value={item.notes} />{profile ? <><Info label="Equipment" value={`${profile.equipmentType}${profile.brand ? ` · ${profile.brand}` : ""}${profile.model ? ` ${profile.model}` : ""}`} /><Info label="Assigned aquariums" value={attachedAquariums || "Not attached"} />{vesselAquariums ? <Info label="Used as vessel for" value={vesselAquariums} /> : null}<Info label="Shared equipment" value={profile.multiAquariumCapable ? "Can serve multiple aquariums" : item.aquariumAttachments.length > 1 ? "Assigned to multiple aquariums without shared flag" : "Single-aquarium by default"} /><Info label="Serial" value={profile.serialNumber} /><Info label="Warranty" value={profile.warrantyUntil ? format(profile.warrantyUntil, "MMM d, yyyy") : null} /><Info label="Next maintenance" value={nextMaintenance ? `${format(nextMaintenance, "MMM d, yyyy")} (${differenceInCalendarDays(nextMaintenance, new Date())}d)` : null} />{profile.equipmentType === "LIGHT" ? <LightLoad item={item} /> : null}</> : null}</CardContent></Card><QuickActions item={item} aquariumId={aquariumId} conditionId={conditions[0]?.id ?? null} canOperate={canOperate} aquariums={aquariums} locations={locations} quarantines={quarantines} basePath={basePath} /></section> : null}
