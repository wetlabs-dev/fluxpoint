...
function LightLoad({ item }: { item: any }) { const assignment = item.lightingAssignments[0]; if (!item.equipmentProfile?.maxLumens || !assignment?.schedule) return <Info label="Estimated daily light load" value="Add max lumens and assign a schedule" />; const result = calculateScheduleLightLoad(assignment.schedule.points, assignment.schedule.capabilityProfile, item.equipmentProfile.maxLumens); return <Info label="Estimated daily light load" value={result.displayValue} />; }
