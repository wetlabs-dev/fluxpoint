...
export function MedicationStartForm({ aquariumId, initialVolumeGallons, definitions }: { aquariumId: string; initialVolumeGallons: number | null; definitions: MedicationDefinitionOption[] }) {
  ...
  const recommendedDose = useMemo(() => {
    ...
    if (!definition?.defaultDoseAmount || !definition.dosePerGallons || !Number.isFinite(tankVolume) || tankVolume <= 0) return null;
    return tankVolume / definition.dosePerGallons * definition.defaultDoseAmount;
  }, [definition, volume]);
  ...
  return (
    <form action={startMedicationCourse} className="grid gap-3">
      ...
      <Select name="medicationDefinitionId" required value={definitionId} onChange={(event) => setDefinitionId(event.target.value)}>
        ...
        {definitions.map((entry) => <option key={entry.id} value={entry.id}>{entry.name}{entry.defaultDoseAmount && entry.dosePerGallons ? ` · ${entry.defaultDoseAmount}${entry.defaultDoseUnit ?? ""} per ${entry.dosePerGallons} gal` : ""}</option>)}
      </Select>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <Input name="tankVolumeGallons" type="number" min="0.1" step="0.1" placeholder="Tank volume used" value={volume} onChange={(event) => setVolume(event.target.value)} required />
      ...
      <div className="rounded-md border border-amber-200 bg-amber-50 p-3 text-xs text-amber-950 dark:border-amber-700 dark:bg-amber-950/30 dark:text-amber-100">Confirm the calculated recommendation against the product label, then edit the actual administered amount if needed. {definition?.safetyNotes ?? "Stored values are planning aids, not medical instructions."}</div>
