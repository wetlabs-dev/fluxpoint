...
export function InhabitantTransferForm({ items, aquariums, storageLocations, quarantineProjects }: any) {
  ...
  const compatibleAquariums = aquariums.filter((aquarium: any) => !selected?.speciesDefinition || speciesMatchesAquariumTarget(aquarium.targetSalinityMinPpt, aquarium.targetSalinityMaxPpt, selected.speciesDefinition.salinityMin, selected.speciesDefinition.salinityMax));
  return (
    <form action={transferItem} className="grid gap-3">
      ...
      <Select name="toAquariumId" defaultValue=""><option value="">Destination aquarium, if applicable</option>{compatibleAquariums.map((entry: any) => <option key={entry.id} value={entry.id}>{entry.generatedName ?? entry.name} · {habitatsForSalinity(entry.targetSalinityMinPpt, entry.targetSalinityMaxPpt).join(" / ").toLowerCase()}</option>)}</Select>
