...
export function EddyAquariumSummary({ aquariumId, aquariumName, provider, fallbackActive, imageEnabled, initialImageUsage, initialImageJob }: { aquariumId: string; aquariumName: string; provider: string; fallbackActive?: boolean; imageEnabled?: boolean; initialImageUsage?: EddyUsageStatus | null; initialImageJob?: any }) {
  ...
  const [imageJob, setImageJob] = useState<any>(initialImageJob ?? null);
  useEffect(() => {
  ...
  return <Card className="overflow-hidden border-water/25">
    ...
    <CardContent className="space-y-5 pt-5">
      ...
      {imageJob ? <div className="rounded-lg border border-water/30 bg-water/10 p-3 text-sm"><div className="flex flex-wrap items-center justify-between gap-2"><strong>{imageJob.progressMessage || imageJob.status}</strong><span className="text-xs text-muted-foreground">{imageJob.priorityLabel} priority · attempt {imageJob.attemptCount}/{imageJob.maxAttempts}</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-muted"><div className="h-full bg-water transition-all" style={{ width: `${imageJob.progress ?? 0}%` }} /></div>{imageJob.nextRetryAt ? <p className="mt-2 text-xs text-muted-foreground">Next retry: {new Date(imageJob.nextRetryAt).toLocaleString()}</p> : null}<details className="mt-3"><summary className="cursor-pointer font-medium">Job activity</summary><ol className="mt-3 space-y-2 border-l border-border pl-4">{(imageJob.events ?? []).map((event: any) => <li key={event.id} className="relative"><span className="absolute -left-[21px] top-1.5 h-2 w-2 rounded-full bg-water" aria-hidden="true"/><div className="flex flex-wrap justify-between gap-2"><span>{event.message}</span><time className="text-xs text-muted-foreground">{new Date(event.createdAt).toLocaleTimeString()}</time></div>{event.attemptNumber ? <div className="text-xs text-muted-foreground">Attempt {event.attemptNumber}{event.details?.model ? ` · ${event.details.provider || "provider"} / ${event.details.model}` : ""}</div> : null}{event.details?.imageUrl && event.eventType === "MEDIA_SAVED" ? <a className="text-xs text-water underline" href={event.details.imageUrl}>View generated image</a> : null}</li>)}</ol></details><div className="mt-3 flex gap-2">{imageJob.status === "PENDING" ? <Button type="button" variant="secondary" onClick={async () => { const response = await fetch(`/api/ai-jobs/${imageJob.id}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "cancel" }) }); setImageJob((await response.json()).job); setImageLoading(false); }}>Cancel</Button> : null}{["FAILED", "DEAD_LETTER"].includes(imageJob.status) ? <Button type="button" variant="secondary" onClick={async () => { const response = await fetch(`/api/ai-jobs/${imageJob.id}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "retry" }) }); setImageJob((await response.json()).job); setImageLoading(true); }}>Retry</Button> : null}</div></div> : null}
