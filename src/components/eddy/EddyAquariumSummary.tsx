...
export function EddyAquariumSummary({ aquariumId, provider, fallbackActive, imageEnabled }: { aquariumId: string; provider: string; fallbackActive?: boolean; imageEnabled?: boolean }) {
  ...
  async function run(action: EddyAction, input: Record<string, unknown> = {}) {
    setActive(action); setLoading(true); setError(null); setResult(null);
    ...
    if (!response.ok) return setError(payload.error || "Eddy could not complete that request.");
    setResult(payload);
  }
  return <Card className="overflow-hidden border-water/25">
    ...
    <CardContent className="space-y-5 pt-5">
      ...
      <div className="rounded-xl border border-border bg-background/45 p-4">{active === "tank-summary" ? <div className="flex flex-wrap items-center justify-between gap-3"><p className="text-sm text-muted-foreground">Review profile, inhabitants, equipment, lighting, readings, timeline, tasks, husbandry, quarantine, and active medication records.</p><Button onClick={() => run("tank-summary")} disabled={loading}><EddyIcon size={18} className="mr-2 h-[18px] w-[18px]" />Summarize tank</Button></div> : null}{active === "compatibility" ? <EddyCompatibilityForm onRun={(input) => run("compatibility", input)} loading={loading} /> : null}{active === "stocking-suggestions" ? <EddyStockingForm onRun={(input) => run("stocking-suggestions", input)} loading={loading} /> : null}{active === "care-recommendations" ? <EddyCareRecommendationPanel onRun={(input) => run("care-recommendations", input)} loading={loading} /> : null}{active === "name-ideas" || active === "cover-concepts" ? <div className="space-y-3"><EddyIdentityGenerator aquariumId={aquariumId} imageEnabled={imageEnabled} onRun={run} loading={loading} /><p className="text-xs text-muted-foreground">Concepts include names, subtitles, palettes, motifs, and a cover prompt. {imageEnabled ? "OpenAI image generation is available from the generated cover workflow." : "Image generation is currently disabled or not configured."}</p></div> : null}{active === "troubleshooting" ? <div className="flex flex-wrap items-center justify-between gap-3"><p className="text-sm text-muted-foreground">Eddy asks for useful missing evidence without pretending to diagnose disease.</p><Button onClick={() => run("troubleshooting")} disabled={loading}><EddyIcon size={18} className="mr-2 h-[18px] w-[18px]" />Start questions</Button></div> : null}</div>
      {loading ? <EddyLoadingState /> : result ? <EddyMessageCard result={result} /> : <EddyEmptyState />}{error ? <p className="text-sm text-destructive">{error}</p> : null}
    </CardContent>
  </Card>;
}
