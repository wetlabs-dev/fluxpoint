...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const job = await prisma.aiJob.findFirst({ where: { id, OR: [{ userId: user.id }, ...(await isServerAdmin(user.id) ? [{}] : [])] }, include: { events: { orderBy: { createdAt: "asc" } } } });
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  else if (body.action === "retry" && (admin || job.userId === user.id)) await retryAiJob(id, admin, user.id);
  else return NextResponse.json({ error: "Unsupported AI job action." }, { status: 400 });
  const updated = await prisma.aiJob.findUniqueOrThrow({ where: { id }, include: { events: { orderBy: { createdAt: "asc" } } } });
