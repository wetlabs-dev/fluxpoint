...
export default async function ConditionDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground"><Link className="font-semibold text-primary underline" href="/conditions">All conditions</Link>{condition.aquarium ? <Link className="font-semibold text-primary underline" href={`/aquariums/${condition.aquarium.id}?workspace=conditions`}>{condition.aquarium.generatedName ?? condition.aquarium.name}</Link> : <span>Collection-wide</span>}<span>{condition.conditionType}</span>{entity ? <span>{entity}</span> : null}</div>
      <section className="grid gap-5 lg:grid-cols-4"><Fact label="First observed" value={format(condition.firstObservedAt, "MMM d, yyyy h:mm a")} /><Fact label="Last observed" value={condition.lastObservedAt ? format(condition.lastObservedAt, "MMM d, yyyy h:mm a") : "Not updated"} /><Fact label="Affected" value={condition.affectedCount === null ? "Not counted" : `${condition.affectedCount} ${condition.affectedCountLabel ?? "affected"}`} /><Fact label="Follow-ups" value={`${condition.careTasks.filter((task) => task.status === "PENDING").length} open`} /></section>
      <section className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_420px]">
        <div className="space-y-6">
          <Card><CardHeader><CardTitle>Observations</CardTitle></CardHeader><CardContent className="space-y-3">{condition.observations.length ? condition.observations.map((observation) => <article key={observation.id} className="rounded-md border border-border bg-background/55 p-4"><div className="flex flex-wrap items-center justify-between gap-2"><div className="font-mono text-xs text-muted-foreground">{format(observation.observedAt, "MMM d, yyyy h:mm a")} · {observation.createdBy?.name ?? "Unknown keeper"}</div><div className="flex gap-2">{observation.status ? <ConditionBadge value={observation.status} /> : null}{observation.severity ? <ConditionBadge value={observation.severity} kind="severity" /> : null}</div></div><p className="mt-2 whitespace-pre-wrap text-sm">{observation.notes}</p>{observation.affectedCount !== null ? <div className="mt-2 text-xs text-muted-foreground">Affected count: {observation.affectedCount}</div> : null}</article>) : <Empty text="No progress observations yet." />}</CardContent></Card>
