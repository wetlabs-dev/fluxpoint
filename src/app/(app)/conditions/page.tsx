...
export default async function ConditionsPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {canCreate ? <CreatePanel title="Log condition" defaultOpen={Boolean(params.create || params.aquariumId || params.entityType || params.entityId)}><ConditionCreateForm timeZone={timeZone} defaults={{ aquariumId: params.aquariumId, entityType: params.entityType, entityId: params.entityId }} aquariums={aquariums.map((entry) => ({ id: entry.id, label: entry.generatedName ?? entry.name }))} items={items.map((entry) => ({ id: entry.id, label: `${entry.name} · ${conditionLabel(entry.itemType)}${entry.aquarium ? ` · ${entry.aquarium.generatedName ?? entry.aquarium.name}` : ""}` }))} species={species.map((entry) => ({ id: entry.id, label: `${entry.commonName}${entry.scientificName ? ` · ${entry.scientificName}` : ""}` }))} /></CreatePanel> : null}
