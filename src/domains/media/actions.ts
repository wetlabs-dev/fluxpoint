...
export async function setAquariumCoverPhoto(formData: FormData) {
  ...
  await writeAuditLog({ entityType: "MediaAsset", entityId: asset.id, action: "PHOTO_SET_AS_COVER", before, after: { aquariumId: asset.aquariumId }, createdById: user.id });
...
export async function hideMediaAsset(formData: FormData) {
  ...
  await writeAuditLog({ entityType: "MediaAsset", entityId: asset.id, action: asset.hiddenAt ? "PHOTO_RESTORED" : "PHOTO_HIDDEN", before: { hiddenAt: asset.hiddenAt }, createdById: user.id });
...
export async function removeMediaAsset(formData: FormData) {
  ...
  await writeAuditLog({ entityType: "MediaAsset", entityId: asset.id, action: "PHOTO_DELETED", before: asset, createdById: user.id });
  if (asset.aquariumId) revalidatePath(`/aquariums/${asset.aquariumId}`);
  revalidatePath("/aquariums");
  revalidatePath("/dashboard");
}
